...
const ja: LocaleMessages<DefaultLocaleMessageSchema> = {
  ...
  projects: {
    ...
    zephyr: {
      description:
        'ディスクに同期用フォルダを作らず、Finder のサイドバーに Dropbox のファイルを並べる macOS 向けのオープンソースクライアント',
      achievements: [
        'macOS の replicated File Provider を基盤とするため、1 テラバイトのアカウントでもファイルを開くまでローカル容量を消費しません',
        'この Mac が一度もダウンロードしていない画像でも Finder が実際のサムネイルを表示し、外部での変更は数秒で反映されます',
        'すべての転送を Dropbox のコンテンツハッシュで検証し、分割アップロードはプロセスが落ちても再開できます',
      ],
    },
    measurementKit: {
      description:
        'Foundation の Measurement 型に欠けている演算・単位・次元を補う Swift ライブラリ',
      achievements: [
        '60 個の次元をまたぐ演算子が、人が書くのと同じ単位で答えます。120 ノットで 2 時間なら 240 海里です',
        'Foundation にない次元（力、勾配、密度、角速度、体積流量・質量流量）を追加します',
        '2 つの温度の除算や、磁方位と真方位の混在はコンパイルエラーになり、気づかない誤りになりません',
      ],
    },
    xcuiTestKit: {
      description:
        'iOS・macOS・watchOS・visionOS の UI テストが遅い CI で不安定にならないようにする、共通の XCUITest ヘルパー',
      achievements: [
        '遅い CI での不安定さの主因であるアクセシビリティサーバーの障害から、アプリを再起動して再試行することで復帰します',
        'フレームが安定してからのタップとキーボードフォーカスの検証により、操作の不安定さの主な原因を取り除きます',
        'タイムアウトは環境変数で倍率を変えられるため、同じテストがローカルでは速く、CI では辛抱強く動きます',
      ],
    },
  },
  resume: {
